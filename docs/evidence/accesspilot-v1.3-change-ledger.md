...
**当前总状态：** `In progress`；T26–T40 `Verified`，T41–T42 `Planned`。T40 已获授权仅本地提交，后续 Ticket 独立验收通过后默认仅本地提交；T39 已本地提交 `3272a4f`。正式简历仍未修改，也不得将尚未完成的完整产品验收能力当作已实现成果。
...
| T40 | 将真实生产图接入 SSE，以 engine-owned terminal finalizer 保持 accepted head/终态/lease 原子顺序；用 worker 屏障处理断线，并在 Workspace 创建时稳定分配双入口 sticky flow | 防止断线误报 interrupted、后台重复执行、JSON/SSE 引擎分裂，以及 live-running 第二个 SSE 先发 200 后静默丢输入 | 可讲解流式传输可靠性、原子 admission、渐进式发布、跨入口恢复与独立验收驱动的并发修复 | [T40 验收证据](accesspilot-v1.3-t40-acceptance-2026-08-20.md)、[SSE/真实 PG 测试](../../apps/api/tests/api/test_t40_sse_langgraph.py)、[生产接线](../../apps/api/src/accesspilot/main.py) | `Verified`；用户已授权仅本地提交；独立验收修复轮 P0/P1/P2=0/0/1，完整 T40 16 项（含官方 PostgresSaver 四向）0 skip，live/expired admission P1 已用 direct-ASGI 反证关闭；cleanup task 异常观察性 P2 不阻塞，尚未进入 T41 |
