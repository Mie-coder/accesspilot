...
**当前总状态：** `In progress`；T26–T35 `Verified`，T36–T42 `Planned`。正式简历仍未修改，也不得将后续未完成的主链、HITL 或轨迹能力当作已实现成果。
...
| T35 | 实现 Workspace 粘性引擎、pending/checkpoint 对账与 Legacy 降级桥 | 防止 JSON/SSE 将同一 Workspace 分流到不同引擎，保证可控回滚 | 可讲解粘性切流、双向对账门禁、不破坏降级与 rollback/resume 并发原子性 | [T35 实现](../tickets/accesspilot-langgraph-agent-loop-v1.3.md#t35--workspace-sticky-enginepending-对账与-legacy-降级桥)、[粘性绑定测试](../../apps/api/tests/agent/test_t35_engine_binding.py)、[对账/降级测试](../../apps/api/tests/agent/test_t35_rollback.py)、[真实 PG 证明](../../apps/api/tests/db/test_t35_postgres.py) | `Verified`；第 2 轮独立验收关闭 R01–R05，P0/P1=0；Codex 实测 T35 定向 64 项（含 8 项隔离真实 PG）与受影响回归 168 项全部通过且无跳过，Ruff/MyPy/diff-check 通过；Pi 报告完整 API 756 项通过、3 项历史环境门控跳过；默认入口仍 Legacy，mixed canary 强制 0 |
