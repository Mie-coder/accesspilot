...
本 Spec 取代同路径下旧的扩张版 v1.2。用户已确认按 T18 → T25 串行实施；T18、T19 已独立验收，T20–T25 仍待开始。

当前交付边界：

- 不把 T20–T25 的目标能力声明为已实现；
- 不修改正式简历；
- 不推送、合并或部署。
...
Ticket 列表已由用户确认，按 T18 → T25 串行实施。T18、T19 已独立验收，当前停止在 T20 开始前；T20–T25 尚未开始。后续实现仍须逐 Ticket 测试先行、独立验收，并保持不推送、不合并、不部署。
