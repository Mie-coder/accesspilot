...
- **开发版本：v1.1** — T09–T14 已完成本地实现与验证；T15–T17 已确认，按顺序待实施。
- **当前能力：** 固定服务端产品身份并隔离受保护的 Demo 控制面，支持按身份发现权限、多意图路由、只读工具白名单、确定性权限名称解析，以及当前轮增量 SSE、Workspace 长连接事件、取消和刷新恢复。
- **后续目标：** 完整政策问答与攻击防护、业务卡片和产品化评测（T15–T17）。
...
- T15–T17：已确认，待按序实施政策问答与攻击防护、业务卡片和产品化评测。
