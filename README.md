...
- **开发版本：v1.1** — T09–T15 已完成本地实现与验证；T16–T17 已确认，按顺序待实施。
- **当前能力：** 固定服务端产品身份并隔离受保护的 Demo 控制面，支持按身份发现权限、多意图路由、只读工具白名单、确定性权限名称解析、8 条基本政策主题问答、基于本轮证据的 `grounded`/`insufficient_evidence`/`retrieval_unavailable` 三态政策回答、自审批禁止规则，以及当前轮增量 SSE、Workspace 长连接事件、取消和刷新恢复。
- **后续目标：** 权限/政策/状态业务卡片和产品化评测（T16–T17）。
