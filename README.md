...
- **开发版本：v1.1** — T09–T13 已完成本地实现与验证；T14–T17 已确认，按顺序待实施。
- **当前能力：** 固定服务端产品身份并隔离受保护的 Demo 控制面，支持按身份发现权限、多意图路由、只读工具白名单、基础安全标记，以及经当前身份目录确定性解析自然语言权限名称。
- **后续目标：** 真实流式响应、完整政策问答、业务卡片和产品化评测（T14–T17）。
...
- T14–T17：已确认，待按序实施真实流式响应、政策问答与攻击防护、业务卡片和产品化评测。
