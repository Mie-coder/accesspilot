...
- **开发版本：v1.1** — T09–T11 已完成本地实现与验证；产品功能书与 Spec 已完成 Claude 首轮评审及 DeepSeek 项目 Key 修订后复核，T12–T17 等待重新确认且尚未实施。
- **当前能力：** 后端绑定演示身份、区分可申请/已拥有权限、多意图路由、只读工具白名单和基础安全标记。
- **后续目标：** 产品/Demo 分层、权限名称解析、真实流式响应、完整政策问答、业务卡片和产品化评测。
...
- T12–T17：交叉评审后已补齐身份终态、预算降级、流式刷新、政策失败状态和三档视口验收，等待用户重新确认；角色切换、模型预算和批量 SSE 仍属于待改造项。
